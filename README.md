...
**Каталог / карточки**
- `catalogList:changed` — перерисовать каталог.
- `card:select` `{ item: IProduct }` — открыть превью товара.

**Корзина**
- `cart:toggle` `{ product: IProduct, inCart?: boolean }` — добавить/удалить позицию.
- `cart:remove` `{ id: string }` — удалить позицию по id.
- `basket:open` — открыть модалку корзины.
- `basket:changed` `{ items: IProduct[], total: number }` — корзина пересчитана.

**Оформление**
- `order:open` — открыть шаг 1 (оплата + адрес).
- `order:step1:submit` — перейти к шагу 2 (контакты).
- `order:submit` — отправить заказ на сервер.
- `order:success:close` — закрыть финальный экран, очистить корзину и счётчик.

**Модалка**
- `modal:open` / `modal:close` — служебные события.  
---

# Потоки данных (User Flow)

1. **Инициализация**  
 `api.loadProducts()` → `catalog.setProducts()` → `emit('catalogList:changed')` → рендер карточек; счётчик корзины синхронится на `basket:changed`.

2. **Просмотр товара**  
   Клик по карточке → `card:select` → `ProductModalView`:
   - цена `null` → «бесценно», кнопка «Недоступно» (`disabled`);
   - иначе — тоггл покупки с `cart:toggle`.

3. **Корзина**  
   `cart:toggle`/`cart:remove` → `Cart` обновляется → `basket:changed`;  
   клик по иконке корзины (`.header__basket`) → `basket:open` → `CartView`.

4. **Оформление**  
   В корзине «Оформить» (если не пусто) → `order:open` → `OrderFormView`;  
   «Далее» при валидных `payment + address` → `order:step1:submit` → `ContactsFormView`;  
   «Оплатить» при валидных `email + phone` → `order:submit`.

5. **POST заказа**  
   Формируется `OrderPayload`:
   - `payment`: `PaymentMethod.Card` → `'card'`, иначе `'cash'`
   - `email`, `phone`, `address`
   - `total = cart.getSubtotal()`
   - `items = cart.getItems().map(p => p.id)`  
   `api.createOrder(payload)` → при `200 OK`:
   - `SuccessView` («Списано N синапсов»);
   - сброс форм: `customer.saveData({ payment: PaymentMethod.Cash, address: '', email: '', phone: '' })`.

6. **Завершение**  
   «За новыми покупками!» → `order:success:close`:
   - `cart.clear()` → `basket:changed` (счётчик = 0),
   - `modal.close()`.
...
- Создать экземпляры: `EventEmitter`, `apiInterpretator`, `Catalog`, `Cart`, `Customer`, `Modal`, `CartView`.
- Найти DOM-узлы (галерея, модалка, кнопка и счётчик корзины, шаблоны).
- Подписаться на события из раздела «Событийная шина».
- Загрузить каталог и инициировать первичный рендер.
- Поддерживать актуальность счётчика корзины и содержимого открытой модалки корзины.
- Отправлять заказ, показывать финальный экран и сбрасывать состояние (формы/корзина) согласно flow.
